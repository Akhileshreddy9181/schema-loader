git clone https://github.com/akhileshreddy9181/$2 /app
cd /app
...
case $1 in
  mongo)
    ...
    --username $(aws ssm get-parameter --name ${env}.docdb.user --with-decryption | jq '.Parameter.Value' | sed -e 's/"//g') --password $(aws ssm get-parameter --name ${env}.docdb.pass --with-decryption | jq '.Parameter.Value' | sed -e 's/"//g') \
    </app/schema/${2}.js
    ;;
  mysql)
